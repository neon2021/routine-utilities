...
def pagination_component(total_records, key_prefix="pagination", default_page_size=20):
    ...
    # 分页大小
    st.session_state[size_key] = st.number_input(
        "每页数量",
        min_value=1,
...
# 使用示例
# total_records = len(result_df) if result_df is not None else 0
# page, page_size = pagination_component(total_records)
# if result_df is not None and not result_df.empty:
#     start, end = (page-1)*page_size, (page-1)*page_size + page_size
#     st.dataframe(result_df[start:end])
# else:
#     st.info("没有数据")
