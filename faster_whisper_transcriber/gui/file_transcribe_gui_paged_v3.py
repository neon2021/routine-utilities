...
def pagination_component(total_records, default_page_size=20, key_prefix="pagination"):
    ...
    # 页码选择，直接绑定 page_key
    st.session_state[page_key] = st.select_slider(
        "选择页码",
        options=list(range(1, total_pages + 1)),
        value=st.session_state[page_key],
        key=page_key  # slider 和按钮共享同一个 key
    )

    # 上一页 / 下一页按钮
    prev_col, next_col = st.columns([1, 1])
    if prev_col.button("上一页", key=f"{key_prefix}_prev") and st.session_state[page_key] > 1:
        st.session_state[page_key] -= 1
    if next_col.button("下一页", key=f"{key_prefix}_next") and st.session_state[page_key] < total_pages:
        st.session_state[page_key] += 1

    st.write(f"总记录数：{total_records}，当前第 {st.session_state[page_key]}/{total_pages} 页")
