...
def handle_result(fut, ctx):
    try:
        res = fut.result()
        logger.info("[id=%s] done: %s", ctx["file_id"], res)
    except Exception as e:
        logger.exception("[id=%s] failed: %r", ctx["file_id"], e)
...
def main():
    ...
    try:
        with conn, conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            ...
            logger.info("Querying candidate media files in id range [%s, %s] ...", args.id_min, args.id_max)
            ...
            with ProcessPoolExecutor(max_workers=MAX_WORKERS, mp_context=ctx) as pool:
                ...
                for r in rows:
                    ...
                    if not filtered:
                        logger.info("[id=%s] no rows after filter; continue.", file_id)
                        continue
                    ...
                    if not cond:
                        logger.debug("[id=%s] IF condition false; skip.", file_id)
                        continue
                    if not path:
                        logger.debug("[id=%s] Empty path; skip.", file_id)
                        continue
                    
                    if not os.path.exists(path):
                        # 文件不存在，跳过
                        logger.info("[id=%s][path=%s] Non-existing file detected; skip.", file_id, path)
                    ...
                    if not is_video_has_audio(path):
                        # 无音频流，跳过
                        logger.info("[id=%s][path=%s] No audio stream detected; skip.", file_id, path)
                ...
                logger.info("-" * 120)
                logger.info("filtered_rows length: %s, count_sum for skipping files: %s", len(filtered_rows), count_sum)
                logger.info("-" * 120)

                for r in filtered_rows:
                    file_id = r["id"]
                    path = r["path"]
                    logger.info("[id=%s] Processing file: %s", file_id, path)
                    ...
                    logger.info("[id=%s] Submitting to pool: %s", file_id, submit_path)
                    ...
                    if len(pending) >= MAX_WORKERS:
                        ...
                        for f in done:
                            try:
                                res = f.result()
                                logger.info("[id=%s] done: %s", ctx_map[f]["file_id"], res)
                            except Exception as e:
