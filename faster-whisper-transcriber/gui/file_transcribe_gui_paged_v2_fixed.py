...
# ================= 统一分页组件 =================
# ================= 改进后的统一分页组件 =================
def pagination_component(total_records, default_page_size=20, key_prefix="pagination"):
    # 初始化 session_state
    ...
    # 读取状态
    page_number = st.session_state[f"{key_prefix}_page_number"]
    page_size = st.session_state[f"{key_prefix}_page_size"]

    # 分页大小选择
    col1, col2 = st.columns(2)
    page_size_option = col1.selectbox(
        "每页数量",
        [10, 20, 50, 100],
        index=[10,20,50,100].index(min(page_size,100)) if page_size in [10,20,50,100] else 1,
        key=f"{key_prefix}_page_size_option"
    )
    page_size_input = col2.number_input(
        "自定义每页数量",
        min_value=1,
    ...
    # 更新 page_size
    st.session_state[f"{key_prefix}_page_size"] = page_size_input
    page_size = page_size_input

    # 计算总页数
    total_pages = max(1, (total_records + page_size - 1) // page_size)
    page_number = min(page_number, total_pages)  # 防止超范围
    ...
    page_number = st.select_slider(
        ...
        value=page_number,
        key=f"{key_prefix}_page_number_slider"
    )
    st.session_state[f"{key_prefix}_page_number"] = page_number

    # 上一页 / 下一页按钮
    prev_col, next_col = st.columns([1, 1])
    if prev_col.button("上一页", key=f"{key_prefix}_prev") and page_number > 1:
        st.session_state[f"{key_prefix}_page_number"] = page_number - 1
    if next_col.button("下一页", key=f"{key_prefix}_next") and page_number < total_pages:
        st.session_state[f"{key_prefix}_page_number"] = page_number + 1

    st.write(f"总记录数：{total_records}，当前第 {st.session_state[f'{key_prefix}_page_number']}/{total_pages} 页")

    return st.session_state[f"{key_prefix}_page_number"], page_size
...
logger.info('File Inventory Explorer')
st.title("File Inventory Explorer")

# ========== 数据源多选 ==========
st.subheader("选择数据源")
selected_dbs = []
data_src_1_col, data_src_2_col = st.columns(2)
with data_src_1_col:
...
for db in selected_dbs:
    ...
    logger.info('before refresh_clicked')
    if refresh_clicked:
        logger.info('after refresh_clicked')
        # 获取最大扫描时间
...
logger.info('统一分页组件')
# 统一分页组件
page_number, page_size = pagination_component(0, key_prefix=db)
